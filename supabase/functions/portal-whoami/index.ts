...
serve(async (req) => {
  ...
  const sessionToken =
    String(req.headers.get("x-portal-session") ?? body.session_token ?? "").trim();
  ...
  if (!sess?.portal_account_id) return json({ ok: false, error: "Unauthorized" }, { status: 401 });
  if (sess.revoked_at) return json({ ok: false, error: "Unauthorized" }, { status: 401 });
  if (new Date(String(sess.expires_at)).getTime() < Date.now()) return json({ ok: false, error: "Unauthorized" }, { status: 401 });

  const portalAccountId = String(sess.portal_account_id);
