...
type Body = { token: string; session_token?: string; agendamento_id: string };
...
serve(async (req) => {
  ...
  if (!token || !sessionToken || !agendamentoId) return json({ ok: false, error: "Unauthorized" }, { status: 401 });
  ...
  const { data: cliente } = await admin
    ...
    .limit(1)
    .maybeSingle();
  if (!cliente?.id) return json({ ok: false, error: "Not allowed" }, { status: 403 });
  const clienteId = String(cliente.id);

  const { data: ag, error: agErr } = await admin
    .from("agendamentos")
    .select("id,funcionario_id,data_hora_inicio,observacoes,status")
    .eq("id", agendamentoId)
    .eq("salao_id", salaoId)
    ...
    .maybeSingle();
  if (agErr) {
    console.error("portal-get-agendamento ag error", agErr);
    return json({ ok: false, error: "Erro ao carregar" }, { status: 400 });
  }
  if (!ag?.id) return json({ ok: false, error: "Not found" }, { status: 404 });
  ...
  const { data: itens, error: itErr } = await admin
    ...
    .limit(10);
  if (itErr) console.error("portal-get-agendamento itens error", itErr);

  return json({
    ok: true,
    agendamento: {
      id: String(ag.id),
      funcionario_id: String(ag.funcionario_id),
      data_hora_inicio: String(ag.data_hora_inicio),
      observacoes: ag.observacoes ?? null,
      status: String(ag.status),
      itens: (itens ?? []).map((i: any) => ({ servico_id: String(i.servico_id) })),
    },
  });
});
