...
serve(async (req) => {
  ...
  if (error) {
    console.error("portal-servicos error", error);
    return json({ ok: false, error: "Erro ao carregar serviços" }, { status: 400 });
  }

  return json({ ok: true, salao_id: salaoId, portal_account_id: portalAccountId, servicos: servicos ?? [] });
});
