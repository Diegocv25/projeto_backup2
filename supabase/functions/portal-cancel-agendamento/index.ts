...
serve(async (req) => {
  ...
  const { data: ag, error: agErr } = await admin
    ...
    .maybeSingle();
  if (agErr) {
    console.error("portal-cancel-agendamento load error", agErr);
    return json({ ok: false, error: "Erro ao cancelar" }, { status: 400 });
  }
  if (!ag?.id) return json({ ok: false, error: "Not found" }, { status: 404 });
  const st = String(ag.status);
  if (st === "cancelado" || st === "concluido") return json({ ok: true });
  ...
  const { error: updErr } = await admin
    ...
    .eq("id", agendamentoId);
  if (updErr) {
    console.error("portal-cancel-agendamento update error", updErr);
    return json({ ok: false, error: "Erro ao cancelar" }, { status: 400 });
  }
