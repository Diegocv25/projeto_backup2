...
type Body = { token?: string; salao_id?: string; session_token?: string; funcionario_id: string; dia: string };
...
serve(async (req) => {
  ...
  if ((!token && !salaoIdFromBody) || !sessionToken || !funcionarioId || !dia) return json({ ok: false, error: "Unauthorized" }, { status: 401 });

  // valida formato yyyy-mm-dd básico
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dia)) return json({ ok: false, error: "Invalid date" }, { status: 400 });
  ...
  // Pega o intervalo do dia em UTC para filtrar timestamptz corretamente
  ...
  const { data: busy, error } = await admin
    ...
    .limit(500);
  if (error) {
    console.error("portal-agendamentos-ocupados error", error);
    return json({ ok: false, error: "Erro ao carregar horários ocupados" }, { status: 400 });
  }

  return json({ ok: true, salao_id: salaoId, portal_account_id: portalAccountId, busy: busy ?? [] });
});
